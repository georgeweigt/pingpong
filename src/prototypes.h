...
uint8_t * create_auth_message(struct node *p, int *plen);
...
uint8_t * encryptmsg(struct node *p, uint8_t *msg, int msglen, int *plen);
int enlength(struct atom *p);
int sublength(struct atom *p);
int padlength(struct atom *p, int sublen);
void generate_ephemeral_keyset(struct node *p);
void init(void);
...
void key_derivation_function(struct node *p);
void list(int n);
...
void send_auth_msg(struct node *p);
